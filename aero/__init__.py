...
def autocomplete(commands, argp):
    if 'AERO_AUTO_COMPLETE' not in os.environ:
        ...
        if '-p' in cwords[cword-1]:
            print ' '.join([p for p in 'less more most cat'.split(' ') if not cwords[cword] or p.startswith(cwords[cword])])
            exit()
        if '-d' in cwords[cword-1]:
            print ' '.join([n for n in adapters if n not in cwords and (not cwords[cword] or n.startswith(cwords[cword]))])
            exit()
        if cwords[cword-1] in commands:
            if not cwords[cword]: print ': '.join(adapters)+': --help'
            else:
        ...
        options = [o for o in options.split(',') if '-d' in o or (o not in cwords and o[1:3] not in cwords and o not in [c[1:3] for c in cwords])]
