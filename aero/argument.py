...
class ArgumentDelegate(argparse.ArgumentParser):
    ...
    def __init__(self, prog='', version=''):
        ...
        self.add_argument(default_prefix+'c', default_prefix*2+'completion',
    ...
    def discover_pager(self, pager=None):
        if not ArgumentDelegate.pager:
            ...
            if pager is not None:
                pagers = [pager] + pagers
            for pager in pagers:
                p = subprocess.Popen(["type", pager], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0]
    ...
    def add_package_commands(self, cmds):
        commargs = self.add_subparsers(metavar='command [mngr:] package',
